...
from model.module.stolr_losses import HSICIndependenceLoss  # 导入 HSIC 独立性损失
...
def main():
    ...
    hsic_loss = HSICIndependenceLoss()  # 实例化 HSIC 独立性损失
    ...
    for epoch in tqdm(range(Config.num_epochs), desc="Training Progress", unit="epoch"):
        ...
        for episode_idx, episode_data in enumerate(episode_loader_with_progress):
            ...
            if use_amp:
                with torch.amp.autocast("cuda", dtype=amp_dtype):
                    ...
                    all_domains = torch.cat(
                        [support_domains, query_domains], dim=0
                    )  # [B_total]
                    domain_logits = outputs["domain_logits"]  # [B_total, 4]
                    domain_loss = F.cross_entropy(domain_logits, all_domains)

                    # 3. HSIC 独立性损失（仅在批次大小 >= 2 时计算）
                    # 注意：HSIC 必须使用未归一化的潜码 u_c 和 u_s，而不是归一化后的 z_c 和 z_s
                    B_total = outputs["u_c"].shape[0]
                    if B_total >= 2:
                        loss_hsic = hsic_loss(outputs["u_c"], outputs["u_s"])
                    else:
                        loss_hsic = torch.tensor(0.0, device=Config.device)

                    # 总损失（使用配置的权重）
                    total_loss = (
                        Config.loss_weight_cls * cls_loss
                        + Config.loss_weight_domain * domain_loss
            ...
            if torch.isnan(total_loss) or torch.isinf(total_loss):
                ...
                print(f"   总损失(total_loss): {total_loss.item()}")
                print(
                    f"   分类损失: {cls_loss.item()}, 域损失: {domain_loss.item()}, HSIC损失: {loss_hsic.item()}"
                )
                raise ValueError(f"总损失为NaN/Inf (total_loss={total_loss.item()})")
