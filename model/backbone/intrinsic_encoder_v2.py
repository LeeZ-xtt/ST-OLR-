...
class FixedSobel(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer(
            "rgb2gray", torch.tensor([0.2989, 0.5870, 0.1140]).view(1, 3, 1, 1)
        )
...
class DropBlock2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mask = (
            torch.rand(
                N, 1, H - self.block_size + 1, W - self.block_size + 1, device=x.device
            )
            < gamma
        ).float()
...
def conv1x1(
    in_planes: int, out_planes: int, stride: int = 1, use_ws: bool = False
) -> nn.Module:
    """1x1 卷积"""
    conv_cls = WSConv2d if use_ws else nn.Conv2d
...
class ResNet12Block(nn.Module):
    ...
    def __init__(
        ...
        use_blurpool: bool = True,  # 是否使用 BlurPool 进行抗锯齿下采样（默认启用）
    ) -> None:
        super().__init__()
        ...
        self.use_blurpool = use_blurpool  # 保存参数用于错误信息

        # 主分支下采样策略：根据 use_blurpool 参数选择下采样算子
        # 注意：如果 keep_resolution=True (dilation 模式)，则不使用下采样
        self.downsample_main = None
        if stride > 1 and not keep_resolution:
            if use_blurpool:
                self.downsample_main = BlurPool2d(planes, stride=stride)
            else:
                self.downsample_main = nn.AvgPool2d(kernel_size=stride, stride=stride)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 注意：当 stride>1 且启用下采样时，shortcut 分支会在 downsample 内下采样。
        # 为保证残差相加形状一致，主分支也必须在相加前完成同样的下采样。
        if self.downsample_main is not None:
            out = self.downsample_main(out)

        if self.downsample is not None:
            identity = self.downsample(x)

        # 形状一致性验证：确保主分支和 shortcut 分支的空间维度匹配
        assert out.shape[-2:] == identity.shape[-2:], (
            f"主分支和shortcut分支的空间维度不匹配: 主分支={out.shape[-2:]}, shortcut={identity.shape[-2:]}, stride={self.stride}"
        )
...
class IntrinsicEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 使用 3x3 小卷积 + GN + LeakyReLU
        self.stem_conv = nn.Sequential(
            nn.Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(8, 64, affine=True),  # 8 组，每组 8 个通道
        ...
        # Stage 2: 160 ch, GN, No WS, DropBlock
    ...
    def _make_layer(
        ...
        # DropBlock
        drop_block = None
        if drop_prob > 0 and self.config.get(
            "use_dropblock", True
        ):  # 默认开启 DropBlock
        ...
        block = ResNet12Block(
            ...
            use_blurpool=self.config.get("use_blurpool", True),  # 传递配置参数
        )
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        Returns:
            pyramid_features: 字典包含
                - "f1": [B, 64, H/2, W/2]
                - "f2": [B, 160, H/4, W/4]
                - "f3": [B, 320, H/8, W/8]
                - "f4": [B, 640, H/8, W/8] (dilated=True)
                       或 [B, 640, H/16, W/16] (dilated=False)
        """
        # 1. Stem
        ...
        x = self.stem_conv(x)  # [B, 64, H, W]

        # 2. Stages - 保存每个阶段的特征图
        f1 = self.layer1(x)  # [B, 64, H/2, W/2]
        f2 = self.layer2(f1)  # [B, 160, H/4, W/4]
        f3 = self.layer3(f2)  # [B, 320, H/8, W/8]
        f4 = self.layer4(f3)  # [B, 640, H/8 or H/16, W/8 or W/16]
    ...
    def get_diagnostics(self) -> dict:
        """返回当前配置诊断信息"""
        return {"drop_rate": self.drop_rate, "dilated": self.dilated, **self.config}
