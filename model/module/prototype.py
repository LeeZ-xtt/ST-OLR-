...
class PrototypeNetwork(nn.Module):
    ...
    def __init__(self, metric: str = "euclidean", temperature: float = 5.0):
        super(PrototypeNetwork, self).__init__()
        self.metric = metric
        self.temperature = temperature

    def compute_prototypes(
        self, support_features: torch.Tensor, support_labels: torch.Tensor, n_way: int
    ) -> torch.Tensor:
        """
        计算每个类别的原型（类别均值）
        ...
        assert support_labels.max() < n_way, (
            f"最大标签值{support_labels.max()}应小于类别数{n_way}"
        )
